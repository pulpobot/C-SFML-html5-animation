...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(400, 400), "Orbit Draw", sf::Style::Titlebar | sf::Style::Close,
                            context);
    ...
    Ball sun(window.getSize().x / 2, window.getSize().y / 2, 100, sf::Color(255, 255, 0));
    sun.mass = 10000;
    particles.emplace_back(sun);

    Ball planet(window.getSize().x / 2 + 200, window.getSize().y / 2, 2, sf::Color(0, 255, 0));
    planet.mass = 1;
    planet.vy = 7;
    particles.emplace_back(planet);
