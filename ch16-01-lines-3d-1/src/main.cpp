...
void Move(Ball3d &ball, float angleX, float angleY, std::vector<sf::Vertex> &lines) {
    RotateX(ball, angleX);
    RotateY(ball, angleY);
    SetPerspective(ball);

    lines.emplace_back(sf::Vertex(sf::Vector2f(ball.GetX(), ball.GetY()), sf::Color::Black));
}
...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(400, 400), "Lines 3D 1", sf::Style::Titlebar | sf::Style::Close,
                            context);
    ...
    for (int i = 0; i < numBalls; ++i) {
        balls.emplace_back(Ball3d(0, 0, 0, 5, sf::Color::Black));
    ...
    while (window.isOpen()) {
        ...
        for (int i = 0; i < numBalls; ++i) {
            Move(balls[i], angleX, angleY, lines);
        }
