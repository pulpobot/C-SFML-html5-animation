...
void Utils::Bezier::QuadraticBezierCurve(const sf::Vector2f &p0, sf::Vector2<float> p1, sf::Vector2<float> p2, int segments, std::vector<sf::Vertex> &curvePoints, sf::Color lineColor, bool throughControlPoint)
{
	//If there are any segments required
	if (segments <= 0)
		return;
