...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(400, 400), "Multi Billiard 1", sf::Style::Titlebar | sf::Style::Close,
                            context);
    ...
    for (int i = 0; i < numBalls; ++i) {
        float radius = dist(m) * 20 + 15;
        balls.emplace_back(Ball(i * 100, i * 50, radius, sf::Color::Red));
        balls[i].mass = radius;
