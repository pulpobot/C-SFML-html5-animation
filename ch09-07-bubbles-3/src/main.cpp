...
void CheckCollision(std::vector<Ball> &balls, int i, sf::RenderWindow &window) {
    for (int j = i + 1; j < balls.size(); ++j) {
        ...
        if (dist < minDis) {
            ...
            float tx = balls[i].GetX() + dx / dist * minDis;
            float ty = balls[i].GetY() + dy / dist * minDis;
            float ax = (tx - balls[j].GetX()) * SPRING;
            float ay = (ty - balls[j].GetY()) * SPRING;
            balls[i].vx -= ax;
            balls[i].vy -= ay;
            balls[j].vx += ax;
            balls[j].vy += ay;
...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(400, 400), "Bubbles 3", sf::Style::Titlebar | sf::Style::Close,
                            context);
    ...
    for (int i = 0; i < numBalls; ++i) {
        balls.emplace_back(
                Ball(dist(mt) * window.getSize().x / 2, dist(mt) * window.getSize().y / 2, dist(mt) * 30 + 20,
